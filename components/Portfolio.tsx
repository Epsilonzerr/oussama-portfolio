    ...
    <main>
      ...
      <section id="top" className="hero wrap">
        ...
        >
          ...
          <h1>
            I build intelligent systems that turn <span>complexity into products.</span>
          </h1>
          <p className="hero-text">
            I&apos;m <strong>Oussama Moustarzik</strong>, a software and AI engineer focused on scalable backend systems,
            AI-powered applications, RAG, intelligent automation and modern web products.
          </p>
        ...
        >
          ...
          <div className="terminal-card">
            ...
            <div><small>CURRENT FOCUS</small><strong>Production AI systems & intelligent developer tools</strong></div>
          </div>
        </motion.div>
      </section>
      ...
      <section id="about" className="section wrap">
        <motion.div {...fadeUp}>
          ...
          <div className="about-grid">
            <h2>Engineering beyond the prompt.</h2>
            <div className="about-copy">
              <p>
                I enjoy building systems where <strong>software engineering and AI meet</strong>: robust APIs, reliable data flows,
                retrieval pipelines, autonomous workflows and clean user experiences.
              </p>
              <p>
                My approach is product-oriented. I care about architecture, observability, maintainability and the quality of the final
                experience — not only whether a model can generate an answer.
              </p>
            </div>
          </div>
          <div className="value-grid">
            <article><BrainCircuit /><h3>Applied AI</h3><p>LLMs, RAG, prompt systems, speech processing and intelligent workflows.</p></article>
            <article><Layers3 /><h3>Backend systems</h3><p>APIs, databases, workers, queues, caching and production-oriented architecture.</p></article>
            <article><Network /><h3>System design</h3><p>Microservices, GraphRAG, distributed components and end-to-end technical design.</p></article>
            <article><Code2 /><h3>Product delivery</h3><p>From technical concept to usable interfaces, deployment and iteration.</p></article>
          </div>
        </motion.div>
      </section>

      <section className="section skills-section wrap">
        <motion.div {...fadeUp}>
          <div className="section-label">02 / TOOLBOX</div>
          <h2 className="medium-heading">Technologies I work with.</h2>
      ...
      <section id="projects" className="section wrap">
        <motion.div {...fadeUp}>
          ...
          <div className="section-heading-row">
            <h2 className="medium-heading">Projects with real technical depth.</h2>
            <p>AI systems, developer tooling, automation and distributed software.</p>
          </div>
        </motion.div>

        <div className="project-list">
          {projects.map((project, index) => (
            <motion.article key={project.name} {...fadeUp} className="project-card">
              <div className="project-index">0{index + 1}</div>
              <div className="project-main">
                <div className="project-meta"><span>{project.label}</span>{project.privateProject && <span>Private / Case study</span>}</div>
                <h3>{project.name}</h3>
                <p>{project.description}</p>
                <div className="highlights">{project.highlights.map((item) => <span key={item}>{item}</span>)}</div>
      ...
      <section id="experience" className="section wrap">
        <motion.div {...fadeUp}>
          ...
          <h2 className="medium-heading">Building, learning, shipping.</h2>
          <div className="timeline">
            {experience.map((item) => (
              <article key={item.role}>
                <div className="period">{item.period}</div>
                <div><h3>{item.role}</h3><span className="company">{item.company}</span><p>{item.description}</p></div>
              </article>
            ))}
          </div>
        </motion.div>
      ...
      <section id="contact" className="contact-section wrap">
        <motion.div {...fadeUp} className="contact-card">
          <div className="contact-icon"><Sparkles size={23} /></div>
          <p className="section-label">05 / CONTACT</p>
          <h2>Have an ambitious project?<br /><span>Let&apos;s build it.</span></h2>
